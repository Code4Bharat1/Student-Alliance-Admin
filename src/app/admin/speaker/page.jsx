...
const CACHE_KEY = "products_speakers";
const CACHE_DURATION = 5 * 60 * 1000; // 5 minutes
const CACHE_METADATA_KEY = "cache_metadata_speakers";
const SEARCH_CACHE_KEY = "search_history_speakers";
const USER_PREFS_KEY = "user_prefs_speakers";
...
export default function Admin() {
    ...
    const fetchProducts = async () => {
      ...
      try {
        const res = await axios.get(
          "https://api-studentalliance.nexcorealliance.com/api/products/category/Speaker"
        );
  ...
  const handleExportCache = () => {
    try {
      ...
      a.href = url;
      a.download = `speakers-cache-${Date.now()}.json`;
      a.click();
          ...
          <div className="relative p-6 bg-white rounded-xl shadow-lg border border-gray-100">
            ...
            {loading && products.length === 0 && (
              <div className="text-center py-12">
                ...
                <p className="mt-4 text-gray-600 font-medium">Loading speakers...</p>
              </div>
            )}
            ...
            {filteredProducts.length === 0 && !loading && (
              <div className="text-center py-12">
                ...
                <p className="text-gray-500 font-medium">
                  {searchTerm ? "No speakers match your search" : "No speakers found"}
                </p>
              </div>
            )}
            ...
            {filteredProducts.length > 0 && !loading && (
              <div className="mt-6 flex items-center justify-between text-sm">
                <span className="text-gray-600">
                  Showing {filteredProducts.length} of {products.length} {products.length === 1 ? "speaker" : "speakers"}
                </span>
                {searchTerm && (
                  <button
